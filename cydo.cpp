...
#include <dlfcn.h>
#define CYDIA_PATH "/var/jb/Applications/Cydia.app/Cydia"

extern "C" void NSLog(CFStringRef, ...);
...
int posix_spawn_root(int argc, char *argv[], char *envp[])
{
    ...
    int pid = 0;
    int status = 0;
    int ret = ie_posix_spawnp(&pid, "/var/jb/usr/libexec/cydia/cydo", NULL, &attr, argv, envp);
    posix_spawnattr_destroy(&attr);
    if (ret)
    {
        NSLog(CFSTR("cannot posix_spawnp(), errno=%d\n"), ret);
        return -1;
    }
    waitpid(pid, &status, 0);
    if (WIFEXITED(status))
    {
        NSLog(CFSTR("child exit status: %d\n"), WTERMSIG(status));
        exit(WEXITSTATUS(status));
    }
    else if (WIFSIGNALED(status))
    {
        NSLog(CFSTR("child exited due to signal: %d\n"), WTERMSIG(status));
...
int main(int argc, char *argv[], char *envp[])
{
    ...
    if (getuid() != 0)
    {
        ...
        {
            if (strncmp(parent_path, "/private/preboot/", sizeof("/private/preboot/")-1)) {
                fprintf(stderr, "nope\n");
                return EX_NOPERM;
            }
        }
    ...
    for (int i = 0; argv[i] != NULL; i++) {
        NSLog(CFSTR("argv[%d]: %s"), i, argv[i]);
    }

    ie_execv(argv[0], argv);

    NSLog(CFSTR("execv failed: %d"), errno);
